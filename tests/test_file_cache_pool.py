...
def test_get_method(temp_dir):
    ...
    try:
        ...
        assert (
            sample_for_source_info["__sources__"][0].dataset_path == mock_raw_file_store.get_path()
        )
...
def test_get_lazy_method(temp_dir):
...
def test_shared_references(temp_dir):
...
def test_raw_method(temp_dir):
    """Test the 'raw' caching method with DecodeFileStore"""
    pool = FileStoreCachePool(parent_cache_dir=temp_dir, method="raw")
    mock_raw_file_store = MockFileStore(
        {
            "file1": b"test data 1",
        }
...
def test_pickle_method(temp_dir):
    """Test the 'pickle' caching method"""
    pool = FileStoreCachePool(parent_cache_dir=temp_dir, method="pickle")
    mock_raw_file_store = MockFileStore(
        {
            "file1": b"test data 1",
        }
    ...
    try:
        ...
        assert result == b"test data 1"
        assert (
            sample_with_source_info["__sources__"][0].dataset_path == mock_raw_file_store.get_path()
        )
        assert sample_with_source_info["__sources__"][0].index is None
        assert sample_with_source_info["__sources__"][0].shard_name is None
        assert sample_with_source_info["__sources__"][0].file_names == ("file1",)
