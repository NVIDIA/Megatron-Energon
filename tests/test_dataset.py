...
class TestDataset(unittest.TestCase):
    ...
    def test_captioning_dataset(self):
        ...
        with get_loader(ds) as l1, get_loader(ds) as l2:
            iter1 = list(l1)
            iter2 = list(l2)
        assert len(iter1) == 50
        assert len(iter2) == 50
    ...
    def test_current_batch_index(self):
        ...
        with get_loader(
            ...
            assert all(
                all(bi == batch_idx for bi in batch.batch_index) for batch_idx, batch in batches
            )
        ...
        with (
            ...
            assert all(
                all(
                    si
                    == batch_idx
                    + (batch_idx // 4 + ((batch_idx // 2 % 2) + sample_offset) // 2) * 2
                    for sample_offset, si in enumerate(batch.sample_index)
                )
                for batch_idx, batch in batches
            )
    ...
    def test_packing(self):
        ...
        with get_savable_loader(
            ...
            assert all(
                s0.__key__ == s1.__key__ for s0, s1 in zip(samples_r0_cmp, samples_r0_restored)
            )
    ...
    def test_group_batch(self):
        ...
        with get_savable_loader(
            ...
            assert all(
                all(key == batch.caption[0] for key in batch.caption) for idx, batch in batches
            )
