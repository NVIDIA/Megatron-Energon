...
def dataset_path(temp_dir):
    """Create the main dataset directory with test data."""
    ...
    create_text_test_dataset(dataset_path)
...
@pytest.fixture
def checkpoint_dir(dataset_path):
    """Create checkpoint directory for test files."""
    checkpoint_dir = dataset_path / "checkpoints"
    checkpoint_dir.mkdir(exist_ok=True, parents=True)
    return checkpoint_dir


@pytest.fixture
def redist_dir(dataset_path):
    """Create redistribution directory for test files."""
    redist_dir = dataset_path / "redist_checkpoints"
    redist_dir.mkdir(exist_ok=True, parents=True)
    return redist_dir
...
def test_split_parts(dataset_path):
    with open(dataset_path / MAIN_FOLDER_NAME / "split.yaml", "r") as f:
        print(f.read())
    with open(dataset_path / MAIN_FOLDER_NAME / "split2.yaml", "r") as f:
        print(f.read())
...
def test_text_dataset(dataset_path):
    worker_config = WorkerConfig(rank=0, world_size=1, num_workers=0)
...
def test_determinism(dataset_path):
...
def test_determinism_taskencoder(dataset_path):
...
def test_determinism_taskencoder_save_restore(dataset_path):
    class TestTaskEncoder(DefaultTaskEncoder):
        @stateless(restore_seeds=True)
        def encode_sample(self, sample: TextSample) -> TextSample:
            rand_str = (
...
def test_restore_state(dataset_path):
    worker_config = WorkerConfig(rank=0, world_size=1, num_workers=0)
...
def test_restore_state_dist(dataset_path):
...
def test_restore_state_workers(dataset_path):
...
def test_invariance_global_samples(dataset_path):
...
def test_redist(dataset_path, checkpoint_dir, redist_dir):
    scenarios = [
        dict(
            configs=(
                WorkerConfig(rank=0, world_size=2, num_workers=2),
    ...
    for rank_config in scenario["configs"]:
        with get_savable_loader(
            ...
            checkpoint_file = checkpoint_dir / f"state_rank{rank_config.rank}.pt"
    ...
    for scenario in scenarios[1:]:
        ...
        for rank_config in scenario["configs"]:
            state = torch.load(
                str(redist_dir / f"state_rank{rank_config.rank}.pt"), weights_only=False
            )
    ...
    for idx, (global_batches, scenario) in enumerate(zip(global_batches_per_scenario, scenarios)):
