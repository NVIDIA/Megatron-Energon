...
class BaseAggregator(ABC, Generic[T_aggregation_data, T_result]):
    ...
    def on_start(self, aggregator_pool: AggregatorPool) -> None:
        """
        Called exactly once in the aggregator thread before receiving any items.
        """
        pass
...
class AggregatorPool(Generic[T_input_data, T_aggregation_data, T_result]):
    """
    A pool that manages multiple worker threads sending results to
    a single aggregator thread.
    ...
    task_queue: queue.Queue[Optional[T_input_data]]
    result_queue: queue.Queue[Optional[T_aggregation_data]]
    ...
    def __init__(
        ...
        # Queue to pass final aggregator data back to the main thread
        self._final_result_data_queue = queue.Queue()

        # Will store whatever is pulled from _final_data_queue in close()
        self._aggregator_final_result_data: Optional[Any] = None

    def _worker(self, worker_id: int) -> None:
        """Function that runs inside each worker thread."""
        while True:
    ...
    def _aggregator_run(self) -> T_result:
        """
        Function that runs in the aggregator thread.
    ...
    def process(self) -> T_result:
        """
        Starts the worker threads and runs the aggregation in the main thread.
        ...
        for w_id in range(self.num_workers):
            t = threading.Thread(target=self._worker, args=(w_id,), daemon=True)
            t.start()
            workers.append(t)
