...
- {py:meth}`def final_pack(self, samples: List[T_encoded_sample]) -> T_batch_sample] <megatron.energon.TaskEncoder.pre_pack>`: Needed if pre_pack is used. Compresses a group of samples to a single sample.
